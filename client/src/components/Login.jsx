...
const Login = () => {
    ...
    const [UserLogin, { isSuccess, isError, error, isLoading }] = useLoginUserMutation()
    ...
    useEffect(() => {
        if (isSuccess) {
            toast.success("User Login Success", { theme: "dark", position: "top-center" })
            if (user.role === "admin") {
                navigate("/admin")
            } else {
                navigate("/")
    ...
    useEffect(() => {
        if (isError) {
            toast.error(error?.data.message || "Unable to Login", { theme: "dark" })
        }
    }, [isError])
    return <>
        <div>
            ...
            <nav class="navbar bg-blue-600">
                <div class="container-fluid">
                    <a class="md:ms-[150px]" href="">
                        <img className='h-[40px] m-0 p-0' src="https://encrypted-tbn3.gstatic.com/images?q=tbn:ANd9GcTIXOIUHHx0zKuVMDw2tTuLY0QC3TKFrsiAnUB1SlPUx5Zld7hp" alt="" />
                    </a>
                    <div class="flex gap-3 ms-[10px]" id="navbarNavAltMarkup">
                        ...
                        <div class="w-[100px] mt-1 ms-[20px] h-[30px] bg-white cursor-pointer flex items-center justify-center ">
                            <h6 class="text-blue-800 font-semibold p-4">Login</h6>
                        </div>
                        <div className='flex gap-10 ms-5'>
                            <div class="width text-light">
            ...
            <div className='hidden md:flex lg:flex'>
                <div class="h-[45px] flex ps-[60px] bg-white w-full gap-[55px]">
                    <div class="flex text-black text-sm font-semibold mt-3">
                        <h6>Electronics</h6>
                    ...
                    <div class="flex text-black text-sm font-semibold mt-3 me-1">
                        <h6>Offer Zone</h6>
                        <i class="bi bi-chevron-down"></i>
                    </div>
                </div>
            ...
            <div className='bg-slate-300'>
                <div className='flex justify-center pb-10 md:pb-0 w-full h-[100%] lg:h-screen md:h-screen shadow-md shadow-slate-600'>
                    <div className='hidden md:flex lg:flex mt-10 w-[40%] h-full'>
                        <img className='w-full h-[70%] rounded-l-md' src="https://i.ytimg.com/vi/MxTaDhwJDLg/maxresdefault.jpg" alt="" />
                    </div>
                    <div className='bg-white mt-10 h-[100%] md:h-[70%] lg:h-[70%] p-10 rounded-r-md'>
                            ...
                            <form onSubmit={formik.handleSubmit}>
                                ...
                                <div className='flex gap-4'>
                                    <button
                                        ...
                                        {isLoading ? <span className="loading loading-spinner loading-md text-white"></span> : "Login"}
                                    </button>
                                    <Link to='/register' className="btn btn-primary w-36 rounded-full btn-outline my-5">Sign up</Link>
                                </div>
                            </form>
                            <p className='text-black text-sm font-semibold text-center mt-8'>If you Don't have a account <Link to='/register' className='text-blue-500 underline' >Sign Up</Link></p>
                        </div>
                    </div>
                </div>
            </div>
